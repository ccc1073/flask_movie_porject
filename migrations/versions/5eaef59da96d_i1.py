...
Revision ID: 5eaef59da96d
Revises: 
Create Date: 2018-10-05 00:40:24.586776
...
def upgrade():
    ...
    op.create_index(op.f('ix_adminer_addtime'), 'adminer', ['addtime'], unique=False)
    ...
    sa.Column('ip', sa.String(length=100), nullable=True),
    sa.Column('addtime', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['admin_id'], ['adminer.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
...
def downgrade():
    ...
    op.drop_table('movie')
    op.drop_index(op.f('ix_adminer_addtime'), table_name='adminer')
    op.drop_table('adminer')
